...
namespace RoundLibrary
{
	public class RoundedLinearLayout : FrameLayout
	{
		...
		private void Init(Context context, IAttributeSet attrs)
		{
			...
			var attributes = context.ObtainStyledAttributes(attrs, Resource.Styleable.RoundedLinearLayout);

			try
			{
				_radius = attributes.GetDimensionPixelSize(Resource.Styleable.RoundedLinearLayout_corners_radius, 0);
			}
			finally
			{
				attributes.Recycle();
